...
import { AircraftsActions, AircraftsActionsTypes, GetAllAircraftsActionError, GetAllAircraftsActionSuccess, GetDesignedAircraftsActionError, GetDesignedAircraftsActionSuccess, GetDevelopmentAircraftsActionError, GetDevelopmentAircraftsActionSuccess, GetProductionAircraftsAction, GetProductionAircraftsActionError, GetProductionAircraftsActionSuccess, SearchAircraftsActionError, SearchAircraftsActionSuccess } from "./aircrafts.actions";
...
export class AircraftsEffects {
    ...
    getProductionAircraftsEffect: Observable<AircraftsActions> = createEffect(
        () => this.effectActions.pipe(
            ofType(AircraftsActionsTypes.GET_PRODUCTION_AIRCRAFTS),
            mergeMap((action: AircraftsActions) => {
                return this.aircraftService.getProductionAircrafts().pipe(
                    map((aircrafts) => new GetProductionAircraftsActionSuccess(aircrafts)),
                    catchError((err) => of(new GetProductionAircraftsActionError(err.message)))
                )
            })
        )
    );
